...
function loadRecipients(type) {
    ...
    fetch(`../func/get_recipients.php?type=${type}`)
        .then(response => response.json())
        .then(data => {
            const select = document.getElementById('recipientSelect');
            select.innerHTML = '<option value="" disabled selected>Choose recipient</option>';
            
            data.forEach(recipient => {
                const option = document.createElement('option');
                option.value = recipient.id;
                option.textContent = recipient.nama_lengkap;
...
function sendMessage() {
    ...
    const endpoint = type === 'dosen' ? '../func/send_mail_dosen.php' : '../func/send_mail.php';
...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
    ...
    <a href="#" class="nav-link" onclick="openMessageModal()">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Select Recipient Category</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <div class="d-grid gap-2">
                    <button class="btn btn-primary" onclick="loadRecipients('dosen')">
                        <i class="fas fa-chalkboard-teacher me-2"></i>Send to Lecturer
                    </button>
                    <button class="btn btn-primary" onclick="loadRecipients('mahasiswa')">
                        <i class="fas fa-user-graduate me-2"></i>Send to Student
                    </button>
                </div>
            </div>
        </div>
...
<div class="modal" id="messageModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="messageForm">
                    <input type="hidden" id="recipientType" name="recipientType">
                    <div class="mb-3">
                        <label class="form-label">Select Recipient</label>
                        <select class="form-select" name="mail_type" id="recipientSelect" required>
